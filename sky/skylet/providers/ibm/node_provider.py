...
class IBMVPCNodeProvider(NodeProvider):
    ...
    def _load_tags(self):
        ...
        if self.tags_file.is_file():
            ...
            if self._get_node_type(name) == NODE_KIND_HEAD:
                ...
                if node := self.ibm_vpc_client.list_instances(
                    name=name
                ).get_result()["instances"]:
        ...
        if {TAG_RAY_NODE_KIND, TAG_RAY_CLUSTER_NAME}.issuperset(set(filters.keys())):
            ...
            with self.lock:
                ...
                for node_id, node_tags in self.nodes_tags.items():
                    ...
                    if any(
                        item not in node_tags.items() for item in filters.items()
                    ):
                        logger.debug(
    ...
    def _create_instance(self, name, base_config):
        ...
        instance_prototype = {
            ...
            "vpc": {"id": self.vpc_tags["vpc_id"]},
            "image": {"id": base_config["image_id"]},
            "zone": {"name": self.zone},
